            ...
            match this.inner.as_mut().project() {
                ...
                AcceptFutureProj::TlsAccepting { future, service } => match future.poll(cx) {
                    Poll::Ready(Ok(Ok(stream))) => {
                        ...
                        if !span.is_disabled() {
                            let connection = stream.ssl();
                            let (protocol_name, protocol_version) = connection.version_str().split_once('v').map(|(name, ver)| (name.to_ascii_lowercase(), ver)).unzip();
                            let cipher = connection.current_cipher().map(|c| c.standard_name());
